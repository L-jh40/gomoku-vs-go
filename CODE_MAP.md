...
- 环面模式：上下/左右互通（气、连五、禁手、领地、距离全部回绕，AI 只搜索实际 n×n 棋盘）；显示为 n+4 格：四周为镜面复制区，背景向外渐变白、网格线由黑渐白（边缘白 50% 起）、假棋子=50%棋子色+50%棋盘底色，实际棋盘四周有更粗的白色镜框；鼠标在复制区时幽灵棋子只显示在实际对应格；点击复制格映射到实际格落子
